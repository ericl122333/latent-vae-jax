...
class ResBlockNoFFN(nn.Module):
    ...
    def __call__(self, inputs, label=None):
        x = inputs
        ksize = 3 if self.is_conv else 1
        ...
        if self.sr_lam is not None:
            x, sr_loss = ConvSR(self.c_out, 3, w_scale=self.w_scale, sr_lam=self.sr_lam)(nn.swish(x))            
            return x, sr_loss
        else:
            x = Conv2D(self.c_out, 3, w_scale=self.w_scale)(nn.swish(x))            
            if self.c==self.c_out:  x += inputs
            return x
...
class ResBlockFFN(nn.Module):
    ...
    def __call__(self, inputs, label=None):
        ...
        x = Conv2D(self.c * self.dff_mul, 1)(nn.swish(x))
        if self.sr_lam is not None:
            x, sr_loss = ConvSR(self.c_out, 1, w_scale=self.w_scale, sr_lam=self.sr_lam)(nn.swish(x))            
            return x, sr_loss
        else:
            x = Conv2D(self.c_out, 1, w_scale=self.w_scale)(nn.swish(x))            
            if self.c==self.c_out:  x += inputs
            return x
...
class AttentionLayer(nn.Module):
    ...
    def __call__(self, x, label=None):
        ...
        if num_heads==1:
            ...
            attn_out = jnp.reshape(q, [B, H*W, num_heads * dhead])
